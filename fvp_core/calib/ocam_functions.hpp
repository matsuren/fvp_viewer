...
//    projects a 3D point (point3D) on to the image and returns the pixel
//coordinates (point2D).
//
...
//    Author: Davide Scaramuzza - email: davide.scaramuzza@ieee.org
//
//    NOTE: the coordinates of "point2D" and "center" are already according to the
//C convention, that is, start from 0 instead than from 1.
//------------------------------------------------------------------------------*/
...
//    where (x^2 + y^2 + z^2) = 1.
//
//    POINT3D = [X;Y;Z] are the coordinates of the 3D points, such that (x^2 + y^2
//+ z^2) = 1. OCAM_MODEL is the model of the calibrated camera. POINT2D =
//[rows;cols] are the pixel coordinates of the point in pixels
//
//    Copyright (C) 2009 DAVIDE SCARAMUZZA
//    Author: Davide Scaramuzza - email: davide.scaramuzza@ieee.org
//
...
//------------------------------------------------------------------------------*/
//void create_perspecive_undistortion_LUT(cv::Mat &mapx, cv::Mat &mapy,
//                                        struct ocam_model *ocam_model,
//                                        float sf);
//void create_perspecive_undistortion_LUT_2ch(cv::Mat &mapxy, int width,
//                                            int height,
//                                            struct ocam_model *ocam_model,
//                                            float sf);
//
...
//------------------------------------------------------------------------------*/
//void create_panoramic_undistortion_LUT(cv::Mat &mapx, cv::Mat &mapy, float Rmin,
//                                       float Rmax, float xc, float yc);
