...
public:
	...
	void update(float t)
	{
		...
		if (abs(angle) > 2 * glm::two_pi<float>() / 3.0 || abs(angle) < glm::radians(120.0f))
			rotSpeed *= -1;
	...
	mat4 getViewMat()
	{
		...
		vec3 cameraPos = vec3(distance * cos(angle) * cos(phi) + lookat_position.x,
			distance * sin(angle)* cos(phi) + lookat_position.y, plus_minus * (distance * sin(phi) + lookat_position.z));
		view = glm::lookAt(cameraPos, lookat_position, vec3(0.0f, 0.0f, plus_minus * 1.0f));
		return view;
	}
	...
	void releasedLeftButton(double x, double y)
	{
		...
		std::cout << "angle : " << angle << ",  " << degree << " degree" << std::endl;
	...
	vec4 getWorldLightPosition() const
	{
		return world_light;
	}
