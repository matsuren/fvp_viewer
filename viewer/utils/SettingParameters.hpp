...
namespace fvp {
	...
	public:
		...
		// 
		ConfigData() : calib_folder("../../data"), record_folder("../../raw_data"),
			image_sources({ "15637060", "15637085","16025862","16025863" }), capture_framerate(20),
			LRF_com_port("COM4"), ocamcalib_files("calib_results_*.txt"),
	...
	public:
		...
		void getRobotPose(cv::Mat &trans_matrix) {
			...
			if (!fs.isOpened()) {
				std::cout << "cann't open file:" << data.robot_align_yml << " in " << data.calib_folder << std::endl;
				return;
			}
			cv::Mat R, rvec, tvec;
			double scale;
		...
		void getLRFPose(double &trans_x, double &trans_y, double &rot_rad) {
			const auto fs = cv::FileStorage(data.calib_folder + "/" + data.lrf_align_yml, cv::FileStorage::READ);
			if (!fs.isOpened()) {
				std::cout << "cann't open file:" << data.lrf_align_yml << " in " << data.calib_folder << std::endl;
				return;
			}
			cv::Mat tvec;
			fs["lrf_rot"] >> rot_rad;
