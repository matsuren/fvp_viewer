...
void GLSLProgram::compileShader(const char *fileName) throw(
...
void GLSLProgram::compileShader(
    const char *fileName,
    GLSLShader::GLSLShaderType type) throw(GLSLProgramException) {
...
void GLSLProgram::compileShader(
    const string &source, GLSLShader::GLSLShaderType type,
...
void GLSLProgram::setUniform(const std::string name,
  ...
  for (int i = 0; i < array_num; i++) {
    ...
    GLint loc = getUniformLocation(element_name.c_str());
    glUniformMatrix4fv(loc, 1, GL_FALSE, &m.at(i)[0][0]);
  }
}
...
void GLSLProgram::setUniform(const char *name, float *val,
                             const int array_num) {
  GLint loc = getUniformLocation(name);
  glUniform1fv(loc, array_num, val);
}

void GLSLProgram::setUniform(const std::string name, double val) {
  setUniform(name.c_str(), val);
}
...
void GLSLProgram::setUniform(const std::string name, double *val,
                             const int array_num) {
  setUniform(name.c_str(), val, array_num);
}

void GLSLProgram::setUniform(const char *name, double *val,
                             const int array_num) {
  GLint loc = getUniformLocation(name);
  float *tmp_val = new float[array_num];
  for (int i = 0; i < array_num; i++) {
    tmp_val[i] = float(val[i]);
  }
  glUniform1fv(loc, array_num, tmp_val);
}
...
void GLSLProgram::printActiveUniformBlocks() {
  ...
  for (int i = 0; i < nBlocks; i++) {
    ...
    GLint nUnis;
    glGetActiveUniformBlockiv(handle, i, GL_UNIFORM_BLOCK_ACTIVE_UNIFORMS,
                              &nUnis);
    GLint *unifIndexes = new GLint[nUnis];
    glGetActiveUniformBlockiv(
        handle, i, GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES, unifIndexes);
    ...
    for (int unif = 0; unif < nUnis; ++unif) {
      ...
      glGetActiveUniform(handle, uniIndex, maxUniLen, &written, &size, &type,
                         uniName);
  ...
  for (int block = 0; block < numBlocks; ++block) {
    GLint blockInfo[2];
    glGetProgramResourceiv(handle, GL_UNIFORM_BLOCK, block, 2, blockProps, 2,
                           NULL, blockInfo);
    GLint numUnis = blockInfo[0];

    char *blockName = new char[blockInfo[1] + 1];
    glGetProgramResourceName(handle, GL_UNIFORM_BLOCK, block, blockInfo[1] + 1,
                             NULL, blockName);
    ...
    for (int unif = 0; unif < numUnis; ++unif) {
      GLint uniIndex = unifIndexes[unif];
      GLint results[3];
      glGetProgramResourceiv(handle, GL_UNIFORM, uniIndex, 3, props, 3, NULL,
                             results);

      GLint nameBufSize = results[0] + 1;
      char *name = new char[nameBufSize];
      glGetProgramResourceName(handle, GL_UNIFORM, uniIndex, nameBufSize, NULL,
                               name);
...
void GLSLProgram::printActiveAttribs() {
  ...
  for (int i = 0; i < numAttribs; ++i) {
    GLint results[3];
    glGetProgramResourceiv(handle, GL_PROGRAM_INPUT, i, 3, properties, 3, NULL,
                           results);
