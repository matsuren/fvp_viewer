...
// private constractor
SensorManager::SensorManager(const std::shared_ptr<fvp::Config> &config)
    : cfg(config) {
...
//-----------------------------------------------------------------------------
void SensorManager::setFVPSystem(std::shared_ptr<fvp::System> system) {
  fvp_system = system;
}
//-----------------------------------------------------------------------------
void SensorManager::startCapture(bool useImshow) {
  ...
  if (useImshow) ths.push_back(std::thread(&SensorManager::viewerWorker, this));
...
//-----------------------------------------------------------------------------
void SensorManager::addCamera(const std::string &image_source,
                              std::map<std::string, std::string> &values) {
...
//-----------------------------------------------------------------------------
void SensorManager::captureWorker(int cam_id, bool enableFPS) {
...
//-----------------------------------------------------------------------------
void SensorManager::LRFWorker(bool enableFPS) {
  std::this_thread::sleep_for(std::chrono::milliseconds(500));
